...
def init_db():
    ...
    """)
    _ensure_column(conn, "channels", "basket_status", "TEXT")
    _ensure_column(conn, "channels", "basket_checked_at", "TEXT")
    _ensure_column(conn, "channels", "basket_error", "TEXT")
    conn.commit()
    conn.close()
...
def _basket_text(status):
    if status == "yes":
        return "🛒 มีตะกร้า"
    if status == "no":
        return "🚫 ไม่พบตะกร้า"
    return "⚠️ เช็กตะกร้าไม่ได้"
...
def send_status_summary():
    ...
    try:
        rows = conn.execute(
            "SELECT username,is_live,last_error,basket_status FROM channels ORDER BY username"
        ).fetchall()
    finally:
        conn.close()
    ...
    for row in rows:
        ...
        if row["last_error"] is not None or row["is_live"] is None:
            ...
            live.append((u, row["basket_status"]))
        else:
            offline.append(u)
    ...
    lines.extend([f"• @{u} — {_basket_text(b)}" for u, b in live] or ["• ไม่มี"])
...
def _get_live_page(username):
    url = f"https://www.tiktok.com/@{username}/live"
    try:
        with httpx.Client(timeout=15, follow_redirects=True, headers=_browser_headers()) as client:
            r = client.get(url)
    except httpx.HTTPError as e:
        return None, None, f"live-page network: {type(e).__name__}"
    if r.status_code in (403, 429):
        return None, None, f"live-page blocked HTTP {r.status_code}"
    if r.status_code != 200:
        return None, None, f"live-page HTTP {r.status_code}"
    low = (r.text or "").lower()
    if any(x in low for x in ("captcha", "verify to continue", "security verification", "access denied", "too many requests")):
        return None, None, "live-page anti-bot response"
    return r, low, None
...
def _check_tiktok_live_page(username):
    r, low, err = _get_live_page(username)
    if err:
        return "unknown", err
...
def check_basket_status(username):
    """Conservative best-effort check from the loaded LIVE page.
    yes = strong shopping/product markers found
    no = LIVE page loaded normally but no shopping markers found
    unknown = page blocked/ambiguous
    """
    r, low, err = _get_live_page(username)
    if err:
        return "unknown", err
    ...
    if final_path != f"/@{username.lower()}/live":
        return "unknown", "ไม่ได้อยู่หน้า LIVE"

    strong_markers = (
        '"product_list"', '"productlist"', '"product_id"', '"productid"',
        '"shop_anchor"', '"shopping_anchor"', '"ecommerce"',
        '"commerce_info"', '"product_card"', '"bag_icon"'
    )
    if any(marker in low for marker in strong_markers):
        return "yes", None

    # Page loaded cleanly but no commerce marker was present.
    return "no", None
...
def check_once():
    ...
    for ch in channels:
        ...
        try:
            ...
            if status == "unknown":
                conn.execute(
                    "UPDATE channels SET last_checked_at=?,last_error=?,basket_status='unknown',basket_checked_at=?,basket_error=? WHERE id=?",
                    (checked, check_error or "ตรวจสถานะไม่ได้", checked, "เช็ก LIVE ไม่สำเร็จ", ch["id"]),
                )
                conn.commit()
                continue

            current, previous = status == "live", ch["is_live"]
            basket_status, basket_error = ("unknown", None)
            if current:
                basket_status, basket_error = check_basket_status(username)
            else:
                basket_status, basket_error = (None, None)

            if previous is None:
                conn.execute(
                    "UPDATE channels SET is_live=?,live_started_at=?,last_checked_at=?,last_changed_at=?,last_error=NULL,basket_status=?,basket_checked_at=?,basket_error=? WHERE id=?",
                    (1 if current else 0, checked if current else None, checked, checked, basket_status, checked, basket_error, ch["id"]),
                )
                conn.commit()
                continue
            ...
            if bool(previous) != current:
                ...
                if current:
                    conn.execute(
                        "UPDATE channels SET is_live=1,live_started_at=?,last_checked_at=?,last_changed_at=?,last_error=NULL,basket_status=?,basket_checked_at=?,basket_error=? WHERE id=?",
                        (changed, checked, changed, basket_status, checked, basket_error, ch["id"]),
                    )
                    conn.execute("INSERT INTO events(channel_id,username,event_type,event_at) VALUES(?,?,?,?)", (ch["id"], username, "START", changed))
                    conn.commit()
                    try:
                        send_telegram(f"🟢 @{username} เริ่ม LIVE แล้ว\n{_basket_text(basket_status)}")
                    except Exception as e:
                    ...
                    if started:
                        try:
                            ...
                            pass
                    conn.execute(
                        "UPDATE channels SET is_live=0,live_started_at=NULL,last_checked_at=?,last_changed_at=?,last_error=NULL,basket_status=NULL,basket_checked_at=?,basket_error=NULL WHERE id=?",
                        (checked, changed, checked, ch["id"]),
                    )
                ...
                conn.execute(
                    "UPDATE channels SET last_checked_at=?,last_error=NULL,basket_status=?,basket_checked_at=?,basket_error=? WHERE id=?",
                    (checked, basket_status, checked, basket_error, ch["id"]),
                )
                conn.commit()
        except Exception as e:
